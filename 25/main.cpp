...
int main()
{
    ...
    char str_num[30]; //para guardar el número convertido a vector de caracteres
    ...
    for(int i=1; i <= m; i++)
    {
        ...
        cin >>  color;
        //PUNTO A: cuántas veces salió el cero
        if (!numero) //es equivalente a if (numero == 0)
        {
            contceros++;
            if (i > 1)
            {
                //itoa(str_num,numanterior);//convierte el int a vector de caracteres
                sprintf(str_num,"%d",numanterior);
        ...
        if (!(numanterior>12 && numanterior<=24)   && !(numero > 12 && numanterior<=24))
            contsegundanegada++;
        else
            contsegundanegada = 0;
